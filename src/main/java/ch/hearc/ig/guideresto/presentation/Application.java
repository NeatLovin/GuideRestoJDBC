...
import ch.hearc.ig.guideresto.persistence.IdentityMapContext;
...
public class Application {
    ...
    // Services uniquement (plus de mappers dans la présentation)
    ...
    private static void addNewRestaurant() {
        try {
            ...
            City city = null;
            do {
                city = pickCity(cityService.findAll());
            } while (city == null);
    ...
    private static void showRestaurant(Restaurant restaurant) {
        try {
            ...
            // Fermer la connexion après avoir terminé avec ce restaurant
            ConnectionUtils.closeConnection();
        }
    }
     ...
     * L'IP locale de l'utilisateur est enregistrée.
     *
    ...
    private static void evaluateRestaurant(Restaurant restaurant) {
        ...
        for (EvaluationCriteria currentCriteria : evaluationCriteriaService.findAll()) {
            ...
            eval.getGrades().add(new Grade(null, note, eval, currentCriteria));
        }

        CompleteEvaluation persisted = evaluationService.createCompleteEvaluation(eval);
        if (persisted != null) {
    ...
    /**
     * Force l'utilisateur à saisir à nouveau toutes les informations du restaurant pour le mettre à jour.
     *
    ...
    private static void editRestaurantAddress(Restaurant restaurant) {
        ...
        City newCity = pickCity(cityService.findAll());
    ...
    /**
     * readInt ne repositionne pas le scanner au début d'une ligne donc il faut le faire manuellement.
     *
